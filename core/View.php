...
class View {
    ...
    public function render($path = '') {
        ...
        if(!file_exists($fullPath)){
            throw new \Exception("The layout \"{$path}\" does not exist");
        }
    ...
    public function start($key){
        if(empty($key)){
            throw new \Exception("Your start method requires a key");
        }
        $this->_buffer = $key;
        ob_start();
    }
    ...
    public function end(){
        if(empty($this->_buffer)){
            throw new \Exception("Your must first run the start method");
        }
